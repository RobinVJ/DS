...
/**
 * Class is a circular linked list. it grows dynamically as per needs.
 * 
 * @author robin
 * 
 * @param <E>
...
public class CircularLinkList<E> implements MyList<E> {
	...
	public CircularLinkList(E[] initialValues) {
		...
		int arrayLen = initialValues.length;
		if (0 == arrayLen) {
			return;
		}
		header = new Link();
		header.data = initialValues[0];
		header.prev = header.next = header;// everything points to header

		Link lastLink = header;
		for (int i = 1; i < arrayLen; i++) {// E value : initialValues) {
			Link newLink = new Link();
			newLink.data = initialValues[i];
	...
	public CircularLinkList(MyList<? extends E> initialValues) {
		...
		int listLen = initialValues.size();
		if (0 == listLen) {
			return;
		}
		header = new Link();
		header.data = initialValues.get(0);
		header.prev = header.next = header;// everything points to header

		Link lastLink = header;
		for (int i = 1; i < listLen; i++) {// E value : initialValues) {
			Link newLink = new Link();
