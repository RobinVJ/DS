...
public class TestCircularLinkList {
	...
	public void createExceptionV1() {
		Integer[] array = null;
		new CircularLinkList<>(array);
	}

	@Test(expected = IllegalArgumentException.class)
	public void createExceptionV2() {
		LinksBasedList<Integer>[] list = null;
		new CircularLinkList<>(list);
	}

	@Test
	public void create() {
		assertEquals(0, new CircularLinkList<>(new Integer[] {}).size());
		assertEquals(0, new CircularLinkList<>(new LinksBasedList<>()).size());
